  ...
  try {
    ...
    const query = `
      ...
      SELECT
        ...
        -- avg daily sales over the window period
        ROUND(
        ...
        -- Returns NULL if avg daily sales is 0 (can't predict stockout
        -- if there's been no selling activity — shouldn't happen given
        -- our WHERE filter, but being safe here).
        CASE
        ...
        -- Supplier info — might be null if no supplier is linked
        s.id                      AS supplier_id,
